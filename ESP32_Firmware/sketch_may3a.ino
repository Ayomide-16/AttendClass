...
// ================= CLOUD SYNC CONFIG =================
const char* CLOUD_BASE_URL = "https://mlpltajakxlmebplrhal.supabase.co";
const char* SYNC_ENDPOINT  = "/functions/v1/hardware-sync";
const char* DEVICE_KEY = "attendesp_device_key_2026";

// ================= DEVICE & CLASS CONFIG =================
const String MASTER_RFID_UID = "2AADAF16"; 
...
/**
 * Uploads a single CSV file from SD card to the cloud backend via HTTPS POST.
 * Streams the file directly from SD to avoid loading it into RAM.
 *
 * @param filePath      The SD card path, e.g. "/attendance.csv"
 * @param displayLabel  A short label for the OLED, e.g. "ATTENDANCE"
 * @return true if the server returned a 2XX response, false otherwise.
 */
bool uploadFileToCloud(const char* filePath, const char* displayLabel) {
  String url = String(CLOUD_BASE_URL) + String(SYNC_ENDPOINT);
  String fileType = String(filePath).indexOf("enrollment") >= 0 ? "enrollment" : "attendance";
  ...
  oledPrint("UPLOADING LOGS", String(displayLabel), "Reading SD...");
  Serial.println("[SYNC] Reading " + String(filePath) + " into memory...");

  // Read the entire 2.5KB file into RAM as one block
  String payload = file.readString();
  file.close();
  
  // IMMEDIATELY release the SD card so it stops competing with Wi-Fi
  xSemaphoreGive(sdMutex); 


  // --- 2. CONNECT & TRANSMIT ---
  oledPrint("UPLOADING LOGS", String(displayLabel), "Sending to Cloud...");
  
  WiFiClientSecure secureClient;
  secureClient.setInsecure(); // Bypass cert validation for hardware

  HTTPClient http;
  http.begin(secureClient, url);
  http.addHeader("Content-Type", "text/csv");
  http.addHeader("x-sync-file-type", fileType);
  http.addHeader("x-device-key", DEVICE_KEY);
  http.setTimeout(20000); // Bump timeout to 20 seconds just in case

  Serial.println("[SYNC] Transmitting...");
  
  // Fire the whole string at once instead of streaming chunks
  int httpCode = http.POST(payload);

  bool success = (httpCode >= 200 && httpCode < 300);

  if (success) {
    // Only take the SD card back to delete the file if successful
    xSemaphoreTake(sdMutex, portMAX_DELAY);
    SD.remove(filePath);
    xSemaphoreGive(sdMutex);
    Serial.println("[SYNC] " + String(displayLabel) + " uploaded & deleted. HTTP " + String(httpCode));
  } else {
    Serial.println("[SYNC] " + String(displayLabel) + " upload FAILED. HTTP " + String(httpCode));
  }

  http.end();
  
  // --- 3. COOL DOWN ---
  // Give the Wi-Fi radio 1 second to fully close the socket before the next file
  delay(1000); 
...
void syncDataToCloud() {
  ...
  // --- Ensure BLE radio is off before enabling Wi-Fi ---
  if (bleReady) {
  ...
  while (WiFi.status() != WL_CONNECTED && retries < 20) {
    delay(500);
    retries++;
    oledPrint("CLOUD SYNC", "Connecting...", "Attempt " + String(retries) + "/20");
  }
  ...
  oledPrint("CLOUD SYNC", "Wi-Fi Connected!", "SYNCING...");
  beepSuccess();

  // --- Upload attendance logs ---
  bool attOk = uploadFileToCloud("/attendance.csv", "ATTENDANCE");

  // --- Upload enrollment logs ---
  bool enrOk = uploadFileToCloud("/enrollment.csv", "ENROLLMENT");
  ...
  if (attOk && enrOk) {
    ...
    String failMsg = "";
    if (!attOk) failMsg += "Attendance ";
    if (!enrOk) failMsg += "Enrollment ";
    oledPrint("SYNC FAILED", failMsg + "error", "Check Serial log.");
    beepError();
  }
  delay(2500);
...
void setup() {
  ...
  // Notice BLE is NO LONGER initialized here in setup!
